...
int32_t uart_dmac_write(UART_AL9000_TypeDef *uart, volatile uint8_t val)
{
   if (__RARELY(uart == NULL)) {
        return -1;
    }
...
{
	uint8_t dma_mode = 0;
    if(dma_mode == AL9000_dma_sw_handshake) {
        printf("you dma handshake error");
    }
    else{
    	dw_dmac_setMemPeriphFlowCtl(Channelx,1,MEM2PER);
    		printf("TX_DMA_MODE");
    		dma_mode = TX_DMA_MODE;
    }
}*/
/**********************************************************************/
/*
void AL9000_uart_setDmaRxMode(UART_AL9000_TypeDef *uart,DMA_Channel_TypeDef *Channelx)
{
    ...
    if(dma_mode == AL9000_dma_sw_handshake) {
    	...
    	dw_dmac_setMemPeriphFlowCtl(Channelx,1,PER2MEM);
    		printf("RX_DMA_MODE");
    		dma_mode = RX_DMA_MODE;
    }
}
*/
/**********************************************************************/
...
int AL9000_uart_userIrqHandler(UART_AL9000_TypeDef *uart)
{
    ...
    int callbackArg;
    uint32_t reg;
    enum AL9000_uart_event event;
    callbackArg = 0;

    // what caused this interrupt
    reg = GET_BITS(uart ->IIR_FCR,0,3);
    event = (enum AL9000_uart_event) (reg & 0xf);
...
int AL9000_uart_haltTx(UART_AL9000_TypeDef *uart)
{
    int retval;
    if(GET_BIT(uart ->CPR,9) == TRUE) {
