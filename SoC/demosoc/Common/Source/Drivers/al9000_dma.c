...
int dw_dmac_reset_all_channels(DMAC_AL9000_TypeDef *DMAC)
{
    int errorCode = 1;

    dw_dmac_disable(DMAC);
    //dw_dmac_disableChannel(DMAC);
    dw_dmac_clearIrq(DMAC);
    ...
    if(errorCode == 1) {
        switch(transfer) {
            case Dmac_transfer_row1 :
                if(errorCode == 1) {
		   dw_dmac_setSingle(Channelx,Dmac_src_dst);
                }
                break;
 	   case Dmac_transfer_row2 :
                if(errorCode == 1) {
		   dw_dmac_setSingle(Channelx,Dmac_src);
		   dw_dmac_setReload(Channelx,Dmac_dst);
                }
                break;
           case Dmac_transfer_row3 :
                if(errorCode == 1) {
		   dw_dmac_setSingle(Channelx,Dmac_dst);
		   dw_dmac_setReload(Channelx,Dmac_src);
                }
                break;
          case Dmac_transfer_row4 :
                if(errorCode == 1) {
		  dw_dmac_setReload(Channelx,Dmac_src_dst);
                }
                break;
          ...
          case Dmac_transfer_row6 :
                if(errorCode == 1) {
		   dw_dmac_set_contigous(Channelx,Dmac_src);
		   dw_dmac_setLlpEnable(Channelx,Dmac_dst);
                }
                break;
          ...
          case Dmac_transfer_row8 :
                if(errorCode == 1) {
		   dw_dmac_set_contigous(Channelx,Dmac_dst);
		   dw_dmac_setLlpEnable(Channelx,Dmac_src);
 	  ...
 	  case Dmac_transfer_row10 :
                if(errorCode == 1) {
		   dw_dmac_setLlpEnable(Channelx,Dmac_src_dst);
                }
                break;
...
/**********************************************************************/
//pass
int dw_dmac_setHsPolarity(DMA_Channel_TypeDef *Channelx,
        enum   dw_dmac_src_dst_select sd_sel,
...
int dma_blocklist_function(uint32_t srcaddress,uint32_t dstaddress,uint32_t llp_address,uint32_t block_number) {
    ...
    //val = dma_block_chain_select(uint32_t srcaddress,uint32_t dma_mode, enum   dw_dmac_transfer_flow  tt_fc)
    if(srcaddress == AL9000_CAN0_BASE ||dstaddress == AL9000_CAN0_BASE || srcaddress == AL9000_CAN1_BASE || dstaddress == AL9000_CAN1_BASE ){
    ...
    if(srcaddress == AL9000_UART0_BASE ||dstaddress == AL9000_UART0_BASE || srcaddress == AL9000_UART1_BASE ||dstaddress == AL9000_UART1_BASE ){
	if(srcaddress == AL9000_UART0_BASE || srcaddress == AL9000_UART1_BASE){
        for (volatile uint8_t i = 0; i < 10; i++) {
            for (volatile uint8_t j = 0; j < 5; j++) {
                switch (j){
                case 0:
	...
	if(dstaddress == AL9000_UART0_BASE || dstaddress == AL9000_UART1_BASE){
        for (volatile uint8_t i = 0; i < 10; i++) {
            for (volatile uint8_t j = 0; j < 5; j++) {
                switch (j){
                case 0:
...
void spi_dma_Init(DMA_Channel_TypeDef *Channelx,enum PER_type PER,enum dma_type type,uint8_t tx_data_level,uint8_t rx_data_level,enum dw_dmac_transfer_type  transfer,uint16_t block_size){
	 dw_dmac_setChannelConfig(Channelx);
        if(type == tx)
	{
	     if(PER == spi0)
		{
    	    dw_dmac_setAddress(Channelx,Dmac_dst,spi0_data_base_address);
	        ...
	        dw_dmac_setAddress(Channelx,Dmac_dst,spi1_data_base_adderss);
		    dam_handshake_interfance(Channelx,spi1_tx_hs_interfance);
		    spi_dma_tx_enable(SPI1);
	        }
    	    dw_dmac_setMemPeriphFlowCtl(Channelx,MEM2PER);
    	    dw_dmac_setAddressInc(Channelx,Dmac_dst,FIX);
    	    spi_dma_data_level(SPI1,tx_data_level,rx_data_level);
	}
    else{
	     if(PER == spi0)
		{
    	    dw_dmac_setAddress(Channelx,Dmac_src,spi0_data_base_address);
		    dam_handshake_interfance(Channelx,spi0_rx_hs_interfance);
		    spi_dma_rx_enable(SPI0);
		    spi_dma_data_level(SPI0,tx_data_level,rx_data_level);
		}
     else if(PER == spi1){
	        dw_dmac_setAddress(Channelx,Dmac_src,spi1_data_base_adderss);
		    dam_handshake_interfance(Channelx,spi1_rx_hs_interfance);
		    spi_dma_rx_enable(SPI1);
	        }
	     dw_dmac_setMemPeriphFlowCtl(Channelx,PER2MEM);
	     dw_dmac_setAddressInc(Channelx,Dmac_src,FIX);
	     spi_dma_data_level(SPI1,tx_data_level,rx_data_level);
	}
     dw_dmac_setTransferType(Channelx,transfer);
	 dw_dmac_setTransWidth(Channelx,Dmac_src_dst,WIDTH_32);
	 dw_dmac_setBlockTransSize(Channelx,block_size);
	 dw_dmac_setBurstTransLength(Channelx,Dmac_src_dst,DW_DMA_MSIZE_1);
}
void qspi_dma_Init(DMA_Channel_TypeDef *Channelx,enum dma_type type,uint8_t tx_data_level,uint8_t rx_data_level,enum dw_dmac_transfer_type  transfer,uint16_t block_size){
	 dw_dmac_setChannelConfig(Channelx);
        if(type == tx)
	{
    	    dw_dmac_setAddress(Channelx,Dmac_dst,qspi_data_base_address);
	    ...
	    dw_dmac_setAddressInc(Channelx,Dmac_dst,FIX);
	     ...
	     dw_dmac_setAddress(Channelx,Dmac_src,qspi_data_base_address);
...
void can_dma_Init(DMA_Channel_TypeDef *Channelx,enum PER_type PER,enum dw_dmac_transfer_type  transfer) {
    dw_dmac_setChannelConfig(Channelx);
	if(PER == can0){
    	    	    dw_dmac_setAddress(Channelx,Dmac_src,AL9000_CAN0_BASE);
		    dam_handshake_interfance(Channelx,can0_rx_hs_interfance);
		    can_dma_enable(AL9000_TOP0,AL9000_CAN0);
	}
    	else if(PER == can1){
	        dw_dmac_setAddress(Channelx,Dmac_src,AL9000_CAN1_BASE);
		dam_handshake_interfance(Channelx,can1_rx_hs_interfance);
		can_dma_enable(AL9000_TOP0,AL9000_CAN1);
	}
    dw_dmac_setTransferType(Channelx,transfer);
    dw_dmac_setMemPeriphFlowCtl(Channelx,PER2MEM);
    dw_dmac_setTransWidth(Channelx,Dmac_src_dst,WIDTH_32);
    dw_dmac_setBlockTransSize(Channelx,20);
    dw_dmac_setBurstTransLength(Channelx,Dmac_src_dst,DW_DMA_MSIZE_4);
    //dw_dmac_setAddressInc(Channelx,Dmac_src,FIX);
}
void uart_dma_Init(DMA_Channel_TypeDef *Channelx,enum PER_type PER,enum dma_type type,enum dw_dmac_transfer_type  transfer,uint16_t block_size) {
    ...
    if(PER == uart0){
 	if(type == tx){
	    dw_dmac_setAddress(Channelx,Dmac_dst,AL9000_UART0_BASE);
	    dam_handshake_interfance(Channelx,uart0_tx_hs_interfance);
    	    dw_dmac_setMemPeriphFlowCtl(Channelx,MEM2PER);
	    dw_dmac_setAddressInc(Channelx,Dmac_dst,FIX);
	}
    	else{
 	     dw_dmac_setAddress(Channelx,Dmac_src,AL9000_UART0_BASE);
	   ...
	   if(type == tx){
	    dw_dmac_setAddress(Channelx,Dmac_dst,AL9000_UART1_BASE);
	    dam_handshake_interfance(Channelx,uart1_tx_hs_interfance);
    	    dw_dmac_setMemPeriphFlowCtl(Channelx,MEM2PER);
	    dw_dmac_setAddressInc(Channelx,Dmac_dst,FIX);
	}
    	else{
 	     dw_dmac_setAddress(Channelx,Dmac_src,AL9000_UART1_BASE);
...
void I2C_dma_Init(DMA_Channel_TypeDef *Channelx,enum PER_type PER,enum dma_type type,uint8_t tx_data_level,uint8_t rx_data_level,enum dw_dmac_transfer_type  transfer,uint16_t block_size) {
    ...
    if(PER == i2c0){
     	if(type == tx){
    	    dw_dmac_setAddress(Channelx,Dmac_dst,i2c0_data_base_address);
    	    ...
    	    i2c_dma_data_level(I2C0,tx_data_level,rx_data_level);
     	     ...
     	     dw_dmac_setAddress(Channelx,Dmac_src,i2c0_data_base_address);
    	   ...
    	   if(type == tx){
    	    dw_dmac_setAddress(Channelx,Dmac_dst,i2c1_data_base_address);
    	    ...
    	    i2c_dma_data_level(I2C1,tx_data_level,rx_data_level);
     	     ...
     	     dw_dmac_setAddress(Channelx,Dmac_src,i2c1_data_base_address);
...
void uart_dma_Init(){
	 ...
	 dw_dmac_setAddress(AL9000_DMAC_channel_0,Dmac_dst,AL9000_UART0_BASE);
	 //dw_dmac_setAddress(AL9000_DMAC_channel_0,dw_dmac_channel_num_1,Dmac_dst,AL9000_UART0_BASE);
	 dw_dmac_setTransferType(AL9000_DMAC_channel_0,Dmac_transfer_row4);
	 dw_dmac_setMemPeriphFlowCtl(AL9000_DMAC_channel_0,MEM2PER);
	 dw_dmac_setTransWidth(AL9000_DMAC_channel_0,Dmac_src_dst,WIDTH_32);
	 dw_dmac_setBlockTransSize(AL9000_DMAC_channel_0,50);
	 dw_dmac_setBurstTransLength(AL9000_DMAC_channel_0,Dmac_src_dst,DW_DMA_MSIZE_32);
	 dw_dmac_setAddressInc(AL9000_DMAC_channel_0,Dmac_dst,FIX);
         //dw_dmac_setLlpEnable(AL9000_DMAC_channel_0,dw_dmac_channel_num_1,Dmac_src_dst);
         //dma_blocklist_function(AL9000_CAN0_BASE,MEM_BASE1_ADDR,LLP_BASE_ADDR);

}
void can_dma_Init(TOPCFG_TypeDef* TOP, enum can_type type, CAN_AL9000_TypeDef* CANX) {
    dw_dmac_setChannelConfig(AL9000_DMAC_channel_0);
    dw_dmac_setAddress(AL9000_DMAC_channel_0,Dmac_src,AL9000_CAN0_BASE);
    dw_dmac_setAddress(AL9000_DMAC_channel_0,Dmac_dst,MEM_BASE1_ADDR);
    //dw_dmac_setTransferType(AL9000_DMAC_channel_0,dw_dmac_channel_num_1,Dmac_transfer_row1,Dmac_src_dst);
    ...
    //dw_dmac_setTransWidth(AL9000_DMAC_channel_0,dw_dmac_channel_num_1,Dmac_src_dst,WIDTH_32);
    //dw_dmac_setBurstTransLength(AL9000_DMAC_channel_0,dw_dmac_channel_num_1,Dmac_src_dst,DW_DMA_MSIZE_4);
    //dw_dmac_setBlockTransSize(AL9000_DMAC_channel_0,dw_dmac_channel_num_1,80);
    dw_dmac_setLlpEnable(AL9000_DMAC_channel_0,Dmac_src_dst);
    dw_dmac_setMemPeriphFlowCtl(AL9000_DMAC_channel_0,PER2MEM);
    //dw_dmac_setAddressInc(AL9000_DMAC_channel_0,dw_dmac_channel_num_1,Dmac_src,FIX);
    can_handshank_function(TOP);
    dma_blocklist_function(AL9000_CAN0_BASE,MEM_BASE1_ADDR,LLP_BASE_ADDR,10);
    //can_dma_mode(AL9000_CAN0,data_length_20);
    can_handshank_function(TOP);
    can_dma_enable(TOP,AL9000_CAN0);
}
