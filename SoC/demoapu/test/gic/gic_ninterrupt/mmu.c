...
static void create_identical_mapping(void)
{
	...
	start = PAGE_ALIGN((unsigned long)0x00000000);
	end =  start + 0x100000;
	__create_pgd_mapping((pgd_t *)idmap_pg_dir, start, start,
			end - start, PROT_NORMAL,
			early_pgtable_alloc,
			0);

	start = PAGE_ALIGN((unsigned long)0x38300000);
	end =  start + 0x100000;
	__create_pgd_mapping((pgd_t *)idmap_pg_dir, start, start,
			end - start, PROT_NORMAL,
			early_pgtable_alloc,
	...
	/*map memory*/
	start = PAGE_ALIGN((unsigned long)_data_start);
	end = (unsigned long)_sp;
	__create_pgd_mapping((pgd_t *)idmap_pg_dir, start, start,
			end - start, PROT_NORMAL,
			early_pgtable_alloc,
			0);
