...
/************************** Function Prototypes ******************************/
static AL_S32 AlAxiUartLite_Test_RecvAndSendPollBlock(AL_VOID);

/************************** Function Definitions ******************************/
/**
 * This function initializes the UART blocked test, executes the test by calling AlAxiUartLite_Test_RecvAndSendPollBlock,
...
AL_S32 main(AL_VOID)
{
    ...
    Ret = AlAxiUartLite_Test_RecvAndSendPollBlock();
    if (Ret != AL_OK) {
...
static AL_S32 AlAxiUartLite_Test_RecvAndSendPollBlock(AL_VOID)
{
    ...
    while (1) {
        Ret = AlAxiUartLite_Hal_RecvDataPolling(UartHandle, Data, BUF_SIZE);
        if (Ret != AL_OK) {
        ...
        if (Ret == AL_OK) {
            Ret = AlAxiUartLite_Hal_SendDataPolling(UartHandle, Data, BUF_SIZE);
            if (Ret != AL_OK) {
