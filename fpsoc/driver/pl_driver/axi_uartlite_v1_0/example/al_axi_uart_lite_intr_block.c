...
/************************** Function Prototypes ******************************/
static AL_S32 AlAxiUartLite_Test_RecvAndSendIntrBlock(AL_VOID);

/************************** Function Definitions ******************************/
/**
 * This function initializes the UART blocked test, executes the test by calling AlAxiUartLite_Test_RecvAndSendIntrBlock,
...
AL_S32 main(AL_VOID)
{
    ...
    AL_LOG(AL_LOG_LEVEL_INFO, "Uart recv and send intr blocked test\r\n");

    Ret = AlAxiUartLite_Test_RecvAndSendIntrBlock();
    if (Ret != AL_OK) {
        AL_LOG(AL_LOG_LEVEL_ERROR, "Uart recv and send intr blocked test failed\r\n");
        return Ret;
    }

    AL_LOG(AL_LOG_LEVEL_INFO, "\r\n");
    AL_LOG(AL_LOG_LEVEL_INFO, "Uart recv and send intr blocked test success\r\n");
    return Ret;
}
...
static AL_S32 AlAxiUartLite_Test_RecvAndSendIntrBlock(AL_VOID)
{
    Al_AxiUartLite_HalStruct *UartHandle;
    AL_U32 ReceiveSize;
    ...
    AL_S32 Ret = AlAxiUartLite_Hal_Init(&UartHandle, 0, AL_NULL);
    if (Ret != AL_OK) {
    ...
    while (1) {
        Ret = AlAxiUartLite_Hal_RecvDataBlock(UartHandle, Data, BUF_SIZE, &ReceiveSize, AL_WAITFOREVER);
        if (Ret != AL_OK) {
        ...
        if (Ret == AL_OK && ReceiveSize > 0) {
            Ret = AlAxiUartLite_Hal_SendDataBlock(UartHandle, Data, ReceiveSize, AL_WAITFOREVER);
            if (Ret != AL_OK) {
