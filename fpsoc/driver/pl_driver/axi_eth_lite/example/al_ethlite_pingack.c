...
static AL_U8 RecvFrame[AL_MAX_FRAME_SIZE];
...
AL_VOID AlGbe_ProcessIcmpPacket(AL_AXI_ETHLITE_HalStruct *AXIETHHandle, AL_U8 *RecvFrame, AL_U32 FrameSize)
{
    AL_U16 *RxFramePtr;
    AL_U16 *TxFramePtr;
    AL_U16 *TempPtr;
            ...
            if (CheckSum == CORRECT_CKSUM_VALUE) {
                ...
                *(IcmpHead + 1) = AL_HTONS(CheckSum);
                ...
                //AlGbe_SendFrame(SendFrame, IpPacketLen + 14);
                    AlAxiEthLite_Hal_SendFrame(AXIETHHandle,
                               (AL_U8 *)&SendFrame,
                               IpPacketLen + 14);
...
AL_VOID AlGbe_ProcessArpPacket(AL_AXI_ETHLITE_HalStruct *AXIETHHandle, AL_U8 *RecvFrame, AL_U32 FrameSize)
{
    AL_U16 *RxFramePtr;
    AL_U16 *TxFramePtr;
    AL_U16 *TempPtr;
...
static void ProcessPingReplyRecvFrame(AL_U32 FrameSize)
{
    AL_U16 *RxFramePtr;
    ...
    if (memcmp(RxFramePtr, BroadcastAddr, ETH_MAC_ADDR_LEN) == 0) {
        AlGbe_ProcessArpPacket(AXIETHHandle, RxFramePtr, FrameSize);
    } else if (memcmp(RxFramePtr, LocalMacAddr, ETH_MAC_ADDR_LEN) == 0) {
        AlGbe_ProcessIcmpPacket(AXIETHHandle, RxFramePtr, FrameSize);
    } else {
        // Do Nothing
    }
...
static int AlAxiEthLite_PingReplyExample()
{
    ...
    while (1) {
        ...
        AlAxiEthLite_Hal_RecvFrame(AXIETHHandle, RecvFrame, &FrameLen);

        if (FrameLen != 0) {
            ProcessPingReplyRecvFrame(FrameLen);
            FrameLen = 0;
        }
    }
}
...
AL_S32 main()
{
    ...
    AL_LOG(AL_LOG_LEVEL_INFO, "AlAxiEthLite ping reply polling example\r\n");
