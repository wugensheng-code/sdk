...
AL_U32 AlAxiQspi_Polled_4Byte_example()
{
    ...
    AL_LOG(AL_LOG_LEVEL_INFO, "Axi-qspi flash test start...\r\n");
    ...
    Status = AlAxiQspi_FlashRead(AlAxiQspiHandle, RecvData, 0x20000, QSPI_FLASH_PAGE_SIZE);
    if (Status != AL_OK) {
        AL_LOG(AL_LOG_LEVEL_ERROR, "Flash read error: 0x%x\r\n", Status);
        return Status;
    }
    ...
    Status = AlAxiQspi_FlashRead(AlAxiQspiHandle, RecvData, 0x00, TestSize);
    if (Status != AL_OK) {
        AL_LOG(AL_LOG_LEVEL_ERROR, "Flash read error: 0x%x\r\n", Status);
        return Status;
    }
    ...
    Status = AlAxiQspi_FlashRead(AlAxiQspiHandle, RecvData, 0x20000, TestSize);
    if (Status != AL_OK) {
        AL_LOG(AL_LOG_LEVEL_ERROR, "Flash read error: 0x%x\r\n", Status);
        return Status;
    }
...
static AL_U32 AlAxiQspi_FlashEraseSector(AlAxiQspi_HalStruct *AlAxiQspiHandle, AL_U32 Addr)
{
    ...
    TempArray[0] = NOR_OP_SE_4B;
    TempArray[1] = (Addr >> 24) & 0xFF;
    TempArray[2] = (Addr >> 16) & 0xFF;
    TempArray[3] = (Addr >> 8) & 0xFF;
    TempArray[4] = Addr & 0xFF;
    Status = AlAxiQspi_Hal_TransferDataPolledBlock(AlAxiQspiHandle, TempArray, 5, AL_NULL, 0, TIMEOUT_FOR_TEST);
    if (Status != AL_OK) {
        return Status;
    }
...
static AL_U32 AlAxiQspi_FlashWritePage(AlAxiQspi_HalStruct *AlAxiQspiHandle, AL_U8 *Data, AL_U32 Addr, AL_U32 Size)
{
    ...
    return AlAxiQspi_Hal_TransferDataPolledBlock(AlAxiQspiHandle, TempArray, Size + 5, AL_NULL, 0, TIMEOUT_FOR_TEST);
}
...
int main(void) {
    RUN_TEST(AlAxiQspi_Polled_4Byte_example);
