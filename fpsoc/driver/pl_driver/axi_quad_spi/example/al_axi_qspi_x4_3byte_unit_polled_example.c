...
static AL_U32 AlAxiQspi_FlashReadConfigReg(AlAxiQspi_HalStruct *AlAxiQspiHandle, AL_U8 *ConfigReg);
static AL_U32 AlAxiQspi_FlashSetConfigReg(AlAxiQspi_HalStruct *AlAxiQspiHandle, AL_U8 *ConfigReg);
...
AL_U32 AlAxiQspi_X4_Polled_3Byte_example()
{
    AL_U8 StatusReg = 0x0U;
    AL_U8 ConfigReg = 0x0U;
    AL_U8 FlashId[3] = {0};
    AL_U32 Status = AL_OK;
    AL_U32 Index = 0;
    ...
    AL_LOG(AL_LOG_LEVEL_INFO, "Axi-qspi x4 3 byte polled mode flash test start...\r\n");
    ...
    AlAxiQspiHandle->Dev.HwConfig.SpiMode = AXI_QSPI_QUAD;
    ...
    // set spi mode to x4
    Status = AlAxiQspi_FlashReadConfigReg(AlAxiQspiHandle, &ConfigReg);
    if (Status != AL_OK) {
        AL_LOG(AL_LOG_LEVEL_ERROR, "Read configuration register error: 0x%x\r\n", Status);
        return Status;
    } else {
        AL_LOG(AL_LOG_LEVEL_INFO, "Read configuration register: 0x%x\r\n", ConfigReg);
    }

    ConfigReg |= (AL_U8)(1 << 1);

    Status = AlAxiQspi_FlashSetConfigReg(AlAxiQspiHandle, &ConfigReg);

    if (Status != AL_OK) {
        AL_LOG(AL_LOG_LEVEL_ERROR, "Set configuration register error: 0x%x\r\n", Status);
        return Status;
    } else {
        AL_LOG(AL_LOG_LEVEL_INFO, "Set configuration register: 0x%x\r\n", ConfigReg);
    }
...
{
    AL_U8 TempArray[8] = {0};
    TempArray[0] = NOR_OP_READ_1_1_4;
    TempArray[1] = (Addr >> 16) & 0xFF;
    TempArray[2] = (Addr >> 8) & 0xFF;
    TempArray[3] = Addr & 0xFF;
    return AlAxiQspi_Hal_TransferDataPolledBlock(AlAxiQspiHandle, TempArray, 8, Data, Size, TIMEOUT_FOR_TEST);
}

static AL_U32 AlAxiQspi_FlashIORead(AlAxiQspi_HalStruct *AlAxiQspiHandle, AL_U8 *Data, AL_U32 Addr, AL_U32 Size)
{
    AL_U8 TempArray[7] = {0};
    TempArray[0] = NOR_OP_READ_1_4_4;
    TempArray[1] = (Addr >> 16) & 0xFF;
    TempArray[2] = (Addr >> 8) & 0xFF;
    TempArray[3] = Addr & 0xFF;
    TempArray[4] = 0x00;
    return AlAxiQspi_Hal_TransferDataPolledBlock(AlAxiQspiHandle, TempArray, 7, Data, Size, TIMEOUT_FOR_TEST);
}
...
static AL_U32 AlAxiQspi_FlashSetConfigReg(AlAxiQspi_HalStruct *AlAxiQspiHandle, AL_U8 *ConfigReg)
{
    AL_U8 TempArray[3] = { 0 };
    AL_U32 Status = AL_OK;
    TempArray[0] = NOR_OP_WRSR;
    TempArray[1] = 0x00;
    TempArray[2] = *ConfigReg;
    ...
    Status = AlAxiQspi_Hal_TransferDataPolledBlock(AlAxiQspiHandle, TempArray, 3, AL_NULL, 0, TIMEOUT_FOR_TEST);
    if (Status != AL_OK) {
        return Status;
    }
...
static AL_U32 AlAxiQspi_FlashReadConfigReg(AlAxiQspi_HalStruct *AlAxiQspiHandle, AL_U8 *ConfigReg)
{
    AL_U8 TempCmd = NOR_OP_RDCR;
    return AlAxiQspi_Hal_TransferDataPolledBlock(AlAxiQspiHandle, &TempCmd, 1, ConfigReg, 1, TIMEOUT_FOR_TEST);
}

int main(void) {
    RUN_TEST(AlAxiQspi_X4_Polled_3Byte_example);
