...
AL_U32 AlAxiQspi_Intr_3Byte_example()
{
...
{
    AL_U8 TempArray[4] = {0};
    TempArray[0] = NOR_OP_READ;
    TempArray[1] = (Addr >> 16) & 0xFF;
    TempArray[2] = (Addr >> 8) & 0xFF;
    TempArray[3] = Addr & 0xFF;
    return AlAxiQspi_Hal_TransferDataBlock(AlAxiQspiHandle, TempArray, 4, Data, Size, TIMEOUT_FOR_TEST);
}
...
static AL_U32 AlAxiQspi_FlashWritePage(AlAxiQspi_HalStruct *AlAxiQspiHandle, AL_U8 *Data, AL_U32 Addr, AL_U32 Size)
{
    ...
    return AlAxiQspi_Hal_TransferDataBlock(AlAxiQspiHandle, TempArray, Size + 4, AL_NULL, 0, TIMEOUT_FOR_TEST);
}
...
int main(void) {
    RUN_TEST(AlAxiQspi_Intr_3Byte_example);
