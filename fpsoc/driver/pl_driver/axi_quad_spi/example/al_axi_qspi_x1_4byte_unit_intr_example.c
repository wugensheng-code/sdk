...
AL_U32 AlAxiQspi_Intr_4Byte_example()
{
...
static AL_U32 AlAxiQspi_FlashEraseSector(AlAxiQspi_HalStruct *AlAxiQspiHandle, AL_U32 Addr)
{
    ...
    Status = AlAxiQspi_Hal_TransferDataBlock(AlAxiQspiHandle, TempArray, 5, AL_NULL, 0, TIMEOUT_FOR_TEST);
    if (Status != AL_OK) {
        return Status;
    }
...
int main(void) {
    RUN_TEST(AlAxiQspi_Intr_4Byte_example);
