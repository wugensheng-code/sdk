...
/**
 * @file    al_adc_polled.c
 * @author  Anlogic esw team
 * @version V0.0.1
 * @date    2023-09-01
 * @brief   adc read data with polled example
 */
...
/************************** Function Prototypes ******************************/
static AL_S32 AlAdc_Test_ReadDataWithPolled(AL_VOID);
...
AL_S32 main(AL_VOID)
{
    ...
    AL_LOG(AL_LOG_LEVEL_INFO, "Adc read data with polled test\r\n");

    Ret = AlAdc_Test_ReadDataWithPolled();
    if (Ret != AL_OK) {
        AL_LOG(AL_LOG_LEVEL_ERROR, "Adc read data with polled test failed\r\n");
        return Ret;
    }

    AL_LOG(AL_LOG_LEVEL_INFO, "Adc read data with polled test success\r\n");
