...
#define AL_MMC_TUNING_BLOCK_OFFSET  (63)
...
static AL_S32 AlMmc_Dev_SetClkPhase(AL_MMC_DevStruct *Dev, AL_U32 Phase)
{
    ...
    Ret = AlMmc_Dev_RstHostController(Dev, AL_MMC_RST_HOST_CMD | AL_MMC_RST_HOST_DAT);
    if (Ret != AL_OK) {
        AL_LOG(AL_LOG_LEVEL_ERROR, "eMMC reset cmd and data line error\r\n");
        return Ret;
    }
}
...
static AL_S32 AlMmc_Dev_SelBusWidthAndSpeed(AL_MMC_DevStruct *Dev)
{
    ...
    AL_S32 Ret = AL_OK;
    AL_U8 CACHE_LINE_ALIGN ReadBuf[AL_MMC_BLK_LEN_512B] = {0};
    ...
    if ((Dev->Config.BusWidth != AL_MMC_BUS_WIDTH_1BIT) && (Dev->CardInfo.SpdMode != AL_MMC_SPD_DS_SDR12)) {
        ...
        Ret = AlMmc_Dev_Write(Dev, TuningBlock, AL_MMC_TUNING_BLOCK_OFFSET, 1);
        if (Ret != AL_OK) {
            AL_LOG(AL_LOG_LEVEL_ERROR, "eMMC tuning data write \033[31mfail\033[0m\r\n");
            goto RETURN;
        }

        Ret = AlMmc_Dev_Read(Dev, ReadBuf, AL_MMC_TUNING_BLOCK_OFFSET, 1);
        if (Ret != AL_OK) {
            AL_LOG(AL_LOG_LEVEL_ERROR, "eMMC tuning data read \033[31mfail\033[0m\r\n");
            goto RETURN;
        }

        Ret = memcmp(TuningBlock, ReadBuf, AL_MMC_BLK_LEN_512B);
        if (Ret != AL_OK) {
            AL_LOG(AL_LOG_LEVEL_ERROR, "eMMC tuning data check \033[31mfail\033[0m\r\n");
            goto RETURN;
        }
    }
...
/**
 * This function initializes the eMMC device in eMMC mode by selecting the optimal clock phase.
 * It asserts that the device structure pointer is not null, iterates through possible clock phases,
 * sets each phase, and tests the bus width and speed for each phase. It logs the results and
 * determines the best phase by finding the largest window of successful phases. Once the best phase
 * is determined, it sets the clock phase to the best value, selects the bus width and speed again,
 * and sets the block size.
 *
 * @param Dev Pointer to the MMC device structure.
 * @return AL_OK if successful, error code otherwise.
 */
static AL_S32 AlMmc_Dev_EmmcModeInit(AL_MMC_DevStruct *Dev)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (Ret != AL_OK) {
            PhaseRes[i] = 0;
            AL_LOG(AL_LOG_LEVEL_ERROR, "eMMC check rx clk phase %d: \033[31mfail\033[0m\r\n", i);
        } else {
            PhaseRes[i] = 1;
            AL_LOG(AL_LOG_LEVEL_INFO, "eMMC check rx clk phase %d: \033[32msuccess\033[0m\r\n", i);
        }
    }
