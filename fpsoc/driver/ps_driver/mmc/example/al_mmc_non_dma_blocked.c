 ...
 * @version V0.0.1
 * @date    2023-09-01
 * @brief   mmc non dma blocked example
 */
...
AL_S32 main(AL_VOID)
{
    ...
    AL_LOG(AL_LOG_LEVEL_INFO, "Mmc non dma blocked test\r\n");

    Ret = AlMmc_Test_NonDmaBlocked();
    if (Ret != AL_OK) {
        AL_LOG(AL_LOG_LEVEL_ERROR, "Mmc non dma blocked test failed\r\n");
        return Ret;
    }

    AL_LOG(AL_LOG_LEVEL_INFO, "Mmc non dma blocked test success\r\n");
