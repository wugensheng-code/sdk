...
AL_S32 AlUart_Hal_Init(AL_UART_HalStruct **Handle, AL_U32 DevId, AL_UART_InitStruct *InitConfig,
                       AL_Uart_EventCallBack Callback)
{
    AL_S32 Ret = AL_OK;
...
/**
 * This function receives data by polling the UART device until the specified amount of data is received or an error
 * occurs.
 *
 * @param Handle A pointer to the UART handle structure.
...
/**
 * This function receives data and waits for a receive complete event or an error event within a specified timeout
 * period.
 *
 * @param Handle A pointer to the UART handle structure.
 * @param Data A pointer to the buffer where received data will be stored.
 ...
 * @return AL_S32 Returns AL_OK on success, or an error code on failure.
 */
AL_S32 AlUart_Hal_RecvDataBlock(AL_UART_HalStruct *Handle, AL_U8 *Data, AL_U32 RequestSize, AL_U32 *ReceiveSize,
                                AL_U32 Timeout)
{
    AL_S32 Ret = AL_OK;
    AL_UART_EventStruct UartEvent = {0};
...
/**
 * This function configures DMA for UART TX, sends data, and waits for a send complete event or an error event within a
