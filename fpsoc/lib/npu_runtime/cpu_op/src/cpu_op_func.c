...
AL_S8 npu_reshape_node_int8(npu_reshape_param_t *param)
{
  AL_S8 *input_buf = NULL;
  AL_S8 *output_buf = NULL;

  // input_buf = (AL_S8 *)param->input_tensor[0]->addr;
  ...
  input_buf = (AL_S8 *)reshape_input->addr;
  output_buf = (AL_S8 *)reshape_output->addr;

  AL_S32 dim_in = reshape_input->dim_num;
  AL_S32 dim_out = reshape_output->dim_num;
  int size = 1;
  ...
  if (reshape_input->layout != 0)
  {
    if (dim_in == 4 && dim_out == 4)
    {
      ...
      AL_S8 *data_int8_temp = (AL_S8 *)malloc(size);
...
AL_S8 npu_reshape_node_uint8(npu_reshape_param_t *param)
{
...
AL_S8 npu_sigmoid_node_int8(npu_sigmoid_param_t *param)
{
  AL_S8 *input_buf = NULL;
  AL_S8 *output_buf = NULL;
  ...
  input_buf = (AL_S8 *)sigmoid_input->addr;
  output_buf = (AL_S8 *)sigmoid_output->addr;

  float input_scale = param->scale_i;
  float output_scale = param->scale_o;
  AL_S32 input_zero = param->zp_i;
  AL_S32 output_zero = param->zp_o;
...
AL_S8 npu_sigmoid_node_uint8(npu_sigmoid_param_t *param)
{
