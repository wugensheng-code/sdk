...
#define LOG_TAG "HARD_NPU_DRIVER"
#include "elog.h"
...
static AL_S32 hard_npu_init(char devDevice[], char sizeDevice[]) {
...
AL_S8 hard_npu_driver_init() {
    ...
    for (int i = 0; i < uio_cnt; i++) {
        ...
        if (strcmp(dev_name, "hard_npu_device") == 0) {
            ...
            if(init_res < 0) {
                log_e("hard npu driver uio init failed\n") ;
                return -1;
            }
            
            hard_irq_fd = open(dev_dir_name, O_RDWR);
            if (hard_irq_fd < 0) {
                log_e("hard_npu device open failed, did you forget to sudo?\n");
                return -1;
            }
        }
    ...
    if(!find_hard_npu) {
        log_e("fail to init hard_npu driver\n");
        return -1;
    }
}
