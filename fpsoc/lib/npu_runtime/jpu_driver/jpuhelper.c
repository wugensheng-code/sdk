...
static int StoreYuvImageBurstFormat_V20(int chromaStride, Uint8 * dst, int picWidth, int picHeight, Uint32 bitDepth, PhysicalAddress addrY, PhysicalAddress addrCb, PhysicalAddress addrCr, int stride, FrameFormat format, int endian, CbCrInterLeave intlv, PackedFormat packed);
...
int jpgGetQMatrix(char *qMatFileName, EncMjpgParam *param)
{
    ...
    if ( qMatFileName[0] != 0 ) {
        ...
        for (i=0; i<64; i++){
            ...
            param->qMatTab[DC_TABLE_INDEX0][i] = lumaQ2[ScanTable[i]] & 0x00ff;
            param->qMatTab[DC_TABLE_INDEX1][i] = lumaQ2[ScanTable[i]] & 0x00ff;
            param->qMatTab[AC_TABLE_INDEX0][i] = chromaBQ2[ScanTable[i]] & 0x00ff;
            param->qMatTab[AC_TABLE_INDEX1][i] = chromaBQ2[ScanTable[i]] & 0x00ff;
        }
    }
