...
Int32	JPU_WaitInterrupt(JpgHandle handle, int timeout)
{
    ...
    if (reason & (1<<INT_JPU_ERROR))  {
        // isseu stop command
        JpuWriteInstReg(instRegIndex, MJPEG_PIC_START_REG, (1<<2));
        if (jdi_wait_pic_status(JPU_INTERRUPT_TIMEOUT_MS, instRegIndex, INT_JPU_STOP_DONE) == -1) {
            JLOG(ERR, "%s jpeg is still working even stop command is issued, status=0x%x\n", __FUNCTION__, JpuReadInstReg(instRegIndex, MJPEG_PIC_STATUS_REG));
            return JPG_RET_FRAME_NOT_COMPLETE;
        }
        JpuWriteInstReg(instRegIndex, MJPEG_PIC_STATUS_REG, 1<<8);
        if (reason & (1<<INT_JPU_ERROR))  {
            Uint32 gbuBitCnt;
            Uint32 elapsedGbuBitCnt;
            Int64 gbuIdleCount = 0;
            Int64 gbuBusyCount = 0;
            const Int64 gbuBusyTimeoutCount = ((Int64)timeout*1000/*micro second*/);
            while(1) {
                gbuBitCnt = JpuReadInstReg(instRegIndex, MJPEG_GBU_BCNT_REG);
                jdi_delay_us(1); // delay as time as many as 200cycle if CCLK is 200Mhz.
                elapsedGbuBitCnt = JpuReadInstReg(instRegIndex, MJPEG_GBU_BCNT_REG);
                if (gbuBitCnt == elapsedGbuBitCnt) {
                    gbuBusyCount = 0;
                    gbuIdleCount++;
                    if (gbuIdleCount >= 3) {
                        JpuWriteInstReg(instRegIndex, MJPEG_BBC_INTERNAL_REG, 0);
                        break;
                    }
                }
                else {
                ...
                if (gbuBusyCount > gbuBusyTimeoutCount) {
                    JLOG(ERR, "%s timeout of gbu busy checking, reason=0x%x \n", __FUNCTION__, reason);
                    return -1;
                }
            }
        }
        if (reason & (1<<INT_JPU_DONE) || reason & (1<<INT_JPU_ERROR) || reason & (1<<INT_JPU_SLICE_DONE))  {
            int respIdleCount = 0;
            int respBusyCount = 0;
            int respBusyTimeoutCount = (timeout*50); 
            while(1) {
                Uint32 rrespFlag;
                Uint32 wrespFlag;
                Uint32 respBusyFlag;
                rrespFlag = JpuReadInstReg(instRegIndex, MJPEG_BBC_BUSY_REG)&0x01;
                wrespFlag = JpuReadInstReg(instRegIndex, MJPEG_WRESP_CHECK_REG)&0x01;
                // JLOG(TRACE, "[%s] %s:%d, rrespFlag=%d, wrespFlag=%d, respIdleCount=%d, respBusyCount=%d \n", __FILE__, __FUNCTION__, __LINE__, rrespFlag, wrespFlag, respIdleCount, respBusyFlag); 
                if (pJpgInst->isDecoder == TRUE) {
                    if (reason & (1<<INT_JPU_SLICE_DONE)) {
                        respBusyFlag = wrespFlag;
                    } 
                    else {
                        respBusyFlag = rrespFlag|wrespFlag;
                    }
                }
                else {
                    respBusyFlag = wrespFlag;
                ...
                if (respBusyCount > respBusyTimeoutCount) {
                    JLOG(ERR, "%s timeout of response checking, reason=0x%x \n", __FUNCTION__, reason);
                    return -1;
                }

                jdi_delay_us(20); // delay as time as many as 3000cycle if CCLK is 200Mhz.
            }
        }
    }
    else {
...
JpgRet JPU_DecGetOutputInfo(JpgDecHandle handle, JpgDecOutputInfo * info)
{
    ...
    if (reason != 0) {
        if (reason & (1<<INT_JPU_ERROR)) {
            // jpu already is stopped. so clearing MJPEG_PIC_STATUS_REG is not needed
            val = reason & ~(1<<INT_JPU_ERROR);
            JpuWriteInstReg(instRegIndex, MJPEG_PIC_STATUS_REG, val);
        }
        else {
