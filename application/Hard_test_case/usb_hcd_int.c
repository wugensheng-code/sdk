...
uint32_t USBH_OTG_ISR_Handler (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  gintsts.d32 = 0;
  printf("USBH_OTG_ISR_Handler!!!\r\n");
  //USB_OTG_BSP_mDelay(10); //if delay 10ms ,cpu is dead
  //printf("1111\n\r");
   // USB_SPD_Get(pdev);
  /* Check if HOST Mode */
...
static uint32_t USB_OTG_USBH_handle_Disconnect_ISR (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  //added by cai, in order to go to disconnect status
  delay_us(100);
  USBH_Connected(pdev);
  return 1;
}
