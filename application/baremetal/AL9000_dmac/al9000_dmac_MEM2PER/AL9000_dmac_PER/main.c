...
/*uart dma channel0 tx test*/
	...
	uart_dma_Init(AL9000_DMAC_channel_0,uart0,tx,Dmac_transfer_row1,50);
	dw_dmac_enable(AL9000_DMAC);
...
/*can0 tx can1 rx dma channel1 rx test*/
	...
	can_dma_Init(AL9000_DMAC_channel_1,can0,Dmac_transfer_row1);
        dw_dmac_enable(AL9000_DMAC);
	...
	while(dw_dmac_checkChannelBusy(AL9000_DMAC));
        dw_dmac_disable(AL9000_DMAC);
        dw_dmac_disableChannel(AL9000_DMAC,dw_dmac_channel_num_1);
	...
	spi_dma_Init(AL9000_DMAC_channel_2,spi0,tx,32,3,Dmac_transfer_row1,64);
	spi_dwc_ssi_enable(SPI0);
	...
	spi_dma_Init(AL9000_DMAC_channel_2,spi0,rx,32,3,Dmac_transfer_row1,10);
	spi_dwc_ssi_enable(SPI0);
	...
	qspi_dma_Init(AL9000_DMAC_channel_3,tx,40,10,Dmac_transfer_row1,64);
	dwc_ssi_enable(QSPI0);
	...
	qspi_data_transmit(QSPI0,0x00); // tx addr[7:0]
        dw_dmac_enable(AL9000_DMAC);
        dw_dmac_enableChannel(AL9000_DMAC,dw_dmac_channel_num_3);
	while(!qspi_sr_tfe(QSPI0)); // wait TFE returns to 1
	while(qspi_sr_busy(QSPI0));  // check busy or idle,wait BUSY returns to 0
	while(dw_dmac_checkChannelBusy(AL9000_DMAC));
	...
	dwc_ssi_disable(QSPI0);
        dw_dmac_disable(AL9000_DMAC);
        dw_dmac_disableChannel(AL9000_DMAC,dw_dmac_channel_num_3);
	...
	qspi_dma_Init(AL9000_DMAC_channel_3,tx,64,10,Dmac_transfer_row1,64);
	dwc_ssi_enable(QSPI0);
...
/*FLASH_X1_DFS_8         read data*/
	...
	//qspi_rx_dma_Init(AL9000_DMAC_channel_1,32,10,Dmac_transfer_row1,32);
	qspi_dma_Init(AL9000_DMAC_channel_3,rx,32,10,Dmac_transfer_row1,32);
	/*en ssi*/
	dwc_ssi_enable(QSPI0);
...
//	qspi_sste_dis(QSPI0);
	...
	//qspi_rx_dma_Init(AL9000_DMAC_channel_0,32,10,Dmac_transfer_row1,64);
	qspi_dma_Init(AL9000_DMAC_channel_4,rx,32,10,Dmac_transfer_row1,64);
	...
	while(!qspi_sr_tfe(QSPI0)); // wait TFE returns to 1
	dw_dmac_enable(AL9000_DMAC);
        dw_dmac_enableChannel(AL9000_DMAC,dw_dmac_channel_num_4);
	while(dw_dmac_checkChannelBusy(AL9000_DMAC));
...
//	FLASH_X2  // this type flash not support X2 Page Program
	...
	dwc_ssi_disable(QSPI0);
	dw_dmac_disable(AL9000_DMAC);
        dw_dmac_disableChannel(AL9000_DMAC,dw_dmac_channel_num_4);
...
//	qspi_sste_dis(QSPI0);
	...
	/*en ssi*/
	//qspi_rx_dma_Init(AL9000_DMAC_channel_0,32,10,Dmac_transfer_row1,64);
	qspi_dma_Init(AL9000_DMAC_channel_5,rx,32,10,Dmac_transfer_row1,64);
	dwc_ssi_enable(QSPI0);
...
//	qspi_data_transmit(QSPI0,0x000000); // tx addr[23:0]
	...
	while(!qspi_sr_tfe(QSPI0)); // wait TFE returns to 1
	dw_dmac_enable(AL9000_DMAC);
        dw_dmac_enableChannel(AL9000_DMAC,dw_dmac_channel_num_5);
	while(dw_dmac_checkChannelBusy(AL9000_DMAC));
	...
	dwc_ssi_disable(QSPI0);
	dw_dmac_disable(AL9000_DMAC);
        dw_dmac_disableChannel(AL9000_DMAC,dw_dmac_channel_num_5);
...
//	qspi_sste_dis(QSPI0);
	...
	/*en ssi*/
	//qspi_tx_dma_Init(AL9000_DMAC_channel_0,32,10,Dmac_transfer_row1,64);
	qspi_dma_Init(AL9000_DMAC_channel_6,tx,32,10,Dmac_transfer_row1,64);
	dwc_ssi_enable(QSPI0);
	...
	dw_dmac_enable(AL9000_DMAC);
        dw_dmac_enableChannel(AL9000_DMAC,dw_dmac_channel_num_6);
	/*check status--wait busy returns to idle*/
	while(!qspi_sr_tfe(QSPI0)); // wait TFE returns to 1
	while(qspi_sr_busy(QSPI0));  // check busy or idle,wait BUSY returns to 0
	//CFG SSIENR
	...
	dwc_ssi_disable(QSPI0);
	dw_dmac_disable(AL9000_DMAC);
        dw_dmac_disableChannel(AL9000_DMAC,dw_dmac_channel_num_6);
...
//	qspi_txftl_tft(QSPI0, 0); // default
	...
	/*en ssi*/
	//qspi_rx_dma_Init(AL9000_DMAC_channel_0,32,10,Dmac_transfer_row1,64);
	qspi_dma_Init(AL9000_DMAC_channel_7,rx,32,10,Dmac_transfer_row1,64);
	dwc_ssi_enable(QSPI0);
...
//	qspi_data_transmit(QSPI0,0x000000); // tx addr[23:0]
	...
	while(!qspi_sr_tfe(QSPI0)); // wait TFE returns to 1
	dw_dmac_enable(AL9000_DMAC);
        dw_dmac_enableChannel(AL9000_DMAC,dw_dmac_channel_num_7);
   // while(1);
	return 0;
}
