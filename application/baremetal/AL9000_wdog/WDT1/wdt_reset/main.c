...
void wdt_handler(void)
{
	...
	if(GET_BIT(WDT1->STAT,0) == 0x1)
	{
		//printf("WDT1->STAT is %x\r\n", WDT1->STAT);
		interrupt_count++;
		if(interrupt_count < 4)
		{
			read_temp = WDT1->EOI;	//Can clear by reading WDT_EOI
			//printf("watch irq %d\r\n", interrupt_count);
			printf("l\r\n");
		}
...
//	}
	...
	ECLIC_Register_IRQ(WDT1_IRQn, ECLIC_NON_VECTOR_INTERRUPT, ECLIC_LEVEL_TRIGGER, 1, 1, wdt_handler);
