...
UDPContext gUdpContext = {
    .ip0 = 10,
    .ip1 = 8,
    .ip2 = 9,
    .ip3 = 4,       // destination ip
    .dstPort = 1234 // 1234 destination port
};

uint32_t initRTPMuxContext(RTPMuxContext *ctx)
{
    ctx->seq = 0;
    ctx->timestamp = 0;
...
void rtpSendData(RTPMuxContext *ctx, const uint8_t *buf, uint32_t len, uint32_t mark)
{
    int res = 0;
...
static void rtpSendNAL(RTPMuxContext *ctx, const uint8_t *nal, uint32_t size, uint32_t last)
{
    ...
    if (size <= RTP_PAYLOAD_MAX)
    {
        ...
        { // printf("1 rtpSendNAL .\r\n");
             ...
             *
             * */
            uint32_t buffered_size = (uint32_t)(ctx->buf_ptr - ctx->buf); // size of data in ctx->buf
            ...
            if (last == 1)
            {
                rtpSendData(ctx, ctx->buf, (uint32_t)(ctx->buf_ptr - ctx->buf), 1);
            }
        }
    ...
    { // 分片分组
        ...
        {
            rtpSendData(ctx, ctx->buf, (uint32_t)(ctx->buf_ptr - ctx->buf), 0);
        }
...
void rtpStreamH264HEVC(RTPMuxContext *ctx, const uint8_t *buf, int size, int last)
{
    static int count = 0;

    // printf("frame[%d] %d\r\n",count++,size);
    if (NULL == ctx || NULL == buf || size <= 0)
    {
    ...
    rtpSendNAL(ctx, buf, size, last);

    // control transmission speed, use as temp. delay
    usleep(1000000 / 25);
    // suppose the frame rate is 25 fps
    ctx->timestamp += (90000.0 / 25);
}

uint32_t rtpSendH264HEVC(RTPMuxContext *ctx, const uint8_t *buf, uint16_t size)
{
    const uint8_t *r;
    ...
    while (r < end)
    {
        ...
        if (r1 == end)
        {
            rtpStreamH264HEVC(ctx, r, (int)(r1 - r), 0);
        }
        else
        {
            rtpStreamH264HEVC(ctx, r, (int)(r1 - r), 1);
        }
