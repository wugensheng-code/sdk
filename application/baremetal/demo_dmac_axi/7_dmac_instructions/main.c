...
int XDmaPs_Example_W_Intr(void)
{
	...
	DmaProgBuf += XDmaPs_Instr_DMALP(DmaProgBuf, 0, 5);
		DmaProgBuf += XDmaPs_Instr_DMASTZ(DmaProgBuf); // store 0
	DmaProgBuf += XDmaPs_Instr_DMALPEND(DmaProgBuf, DmaProgBuf-1, 0);
	DmaProgBuf += XDmaPs_Instr_DMAMOV(DmaProgBuf, 0x1, 0x0019C067);
// Test DMAKILL
	//DmaProgBuf += XDmaPs_Instr_DMAKILL(DmaProgBuf);
		DmaProgBuf += XDmaPs_Instr_DMASTZ(DmaProgBuf);
	DmaProgBuf += XDmaPs_Instr_DMAMOV(DmaProgBuf, 0x1, 0x00084021);
		DmaProgBuf += XDmaPs_Instr_DMASTZ(DmaProgBuf);
	DmaProgBuf += XDmaPs_Instr_DMASEV(DmaProgBuf, 0);
// test DMAADDH、DMAADNH
	DmaProgBuf += XDmaPs_Instr_DMAMOV(DmaProgBuf, 0x0, (u32)Src);
	DmaProgBuf += XDmaPs_Instr_DMAMOV(DmaProgBuf, 0x2, (u32)Dst);
	DmaProgBuf += XDmaPs_Instr_DMAADNH(DmaProgBuf, 0, 0xFFFF);// SAR -1
	DmaProgBuf += XDmaPs_Instr_DMAADDH(DmaProgBuf, 0, 0x3);// SAR +3
	DmaProgBuf += XDmaPs_Instr_DMAADNH(DmaProgBuf, 1, 0xFFFF);// DAR -1
	DmaProgBuf += XDmaPs_Instr_DMAADDH(DmaProgBuf, 1, 0x3);// DAR +3
	...
	while(1){
	  ...
	  printf("** DMAC_AXI_SECURE__INT_EVENT_RIS__ADDR = %x\n",rdata0);
	   if(rdata0 == 0x00000001) break;
		...
		vfwp("** CSR0 = %x \n",rdata0);
		// view SAR0
		rdata0 = REG_READ(0xF8418400);
		vfwp("** SAR0 = %x \n",rdata0);
		// view DAR0
		rdata0 = REG_READ(0xF8418404);
		vfwp("** DAR0 = %x \n",rdata0);
