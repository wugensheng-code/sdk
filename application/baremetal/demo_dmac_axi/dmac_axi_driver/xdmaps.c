...
static int XDmaPs_Exec_DMAKILL(u32 BaseAddr, unsigned int Channel, unsigned int Thread);

static void XDmaPs_BufPool_Free(XDmaPs_ProgBuf *Pool, void *Buf);

static int XDmaPs_Exec_DMAGO(u32 BaseAddr, unsigned int Channel, u32 DmaProg, unsigned int ns);
...
int XDmaPs_CfgInitialize(XDmaPs *InstPtr, XDmaPs_Config *Config, u32 EffectiveAddr)
{
	...
	memset(InstPtr->Chans, 0, sizeof(XDmaPs_ChannelData[XDMAPS_CHANNELS_PER_DEV]));
...
*
*****************************************************************************/
XDmaPs_Instr_DMASEV(char *DmaProg, unsigned int EventNumber)
{
	/*
	 * DMASEV encoding
...
/****************************************************************************/
/**
* @param	Rd is the register id, 0 for SAR, 1 for DAR
* @param	Imm is the 16-bit immediate number
*
* @return 	The number of bytes for this instruction which is 3.
*
*****************************************************************************/
...
u32 XDmaPs_ToCCRValue(XDmaPs_ChanCtrl *ChanCtrl)
{
	...
	//vfwp("src dst prot: === %x %x %x", src_prot_ctrl, dst_prot_ctrl, ccr_value);
...
*
*****************************************************************************/
static int XDmaPs_BuildDmaProg(unsigned Channel, XDmaPs_Cmd *Cmd, unsigned CacheLength)
{
	/*
	 * unpack arguments
	 */
...
#else
	Mem2MemByteCC.DstProtCtrl = (0x1 << 1);
#endif
...
#else
	Mem2MemByteCC.SrcProtCtrl = (0x1 << 1);
#endif
	...
	if (LoopCount > 256) {
		...
		if (LoopCount1 > 1)
			DmaProgBuf += XDmaPs_ConstructNestedLoop(DmaProgStart, CacheLength, DmaProgBuf, LoopCount1,  256);
		else
			DmaProgBuf += XDmaPs_ConstructSingleLoop(DmaProgStart, CacheLength, DmaProgBuf, 256);
	...
	if (TailBytes) {
		...
		if (TailBytes) {
			...
			DmaProgBuf += XDmaPs_ConstructSingleLoop(DmaProgStart, CacheLength, DmaProgBuf,TailBytes);
		}
	}
...
*		buffer.
* @param        ns , what mode is used to start a channel thread. 1 on non-secure, 0 on secure.
*
* @return
...
int XDmaPs_Start(XDmaPs *InstPtr, unsigned int Channel, XDmaPs_Cmd *Cmd, int HoldDmaProg, unsigned int ns)
{
	...
	if (DmaProg) {
		...
		Inten = XDmaPs_ReadReg(InstPtr->Config.BaseAddress, XDMAPS_INTEN_OFFSET);
		Inten |= 0x01 << Channel; /* set the correpsonding bit */
		XDmaPs_WriteReg(InstPtr->Config.BaseAddress, XDMAPS_INTEN_OFFSET,Inten);
		Inten = XDmaPs_ReadReg(InstPtr->Config.BaseAddress, XDMAPS_INTEN_OFFSET);
		...
		Status = XDmaPs_Exec_DMAGO(InstPtr->Config.BaseAddress, Channel, DmaProg, ns);
	} else {
...
* @param        ns 1 on non-secure , 0 on secure
*
* @return	0 on success, -1 on time out
*
****************************************************************************/
static int XDmaPs_Exec_DMAGO(u32 BaseAddr, unsigned int Channel, u32 DmaProg, unsigned int ns)
{
	...
	XDmaPs_Instr_DMAGO(DmaGoProg, Channel, DmaProg, ns);
	...
	if (WaitCount >= XDMAPS_MAX_WAIT) {
		//vfwp("PL330 device at %x debug status busy time out\r\n", BaseAddr);
		return -1;
	}
...
static void XDmaPs_DoneISR_n(XDmaPs *InstPtr, unsigned Channel)
{
	...
	/*Value = XDmaPs_ReadReg(InstPtr->Config.BaseAddress, XDMAPS_INTSTATUS_OFFSET);*/

	/* clear the interrupt status */
	XDmaPs_WriteReg(InstPtr->Config.BaseAddress,XDMAPS_INTCLR_OFFSET, 1 << ChanData->ChanId);
