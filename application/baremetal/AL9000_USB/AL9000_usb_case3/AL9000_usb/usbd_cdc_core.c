                                ...
                                USB_SETUP_REQ *req)
{
      ...
      if (req->wLength)
      {
        ...
        if (req->bmRequest & 0x80)
        {
          /* Get the data to be sent to Host from interface layer */
        	//goto VCP_Ctrl Get uart msg

          APP_FOPS.pIf_Ctrl(req->bRequest, CmdBuff, req->wLength);
          
#if 0
			printf("\r\n usbd_cdc_Setup 0 USBD_CtlSendData %d  { ", req->wLength);
        ...
        else /* Host-to-Device request */
        {
          ...
          function. */
          //printf("\r\n usbd_cdc_Setup USBD_CtlPrepareRx CmdBuff:%p wLength:%d \r\n",CmdBuff, req->wLength);
          USBD_CtlPrepareRx (pdev,
                             CmdBuff,
                             req->wLength);
#if 0
			printf("\r\n usbd_cdc_Setup 0 USBD_CtlPrepareRx %d  { ", req->wLength);
				  for (i=0; i<req->wLength; i++)
					  printf("%02x ", CmdBuff[i]);
			  printf("}\r\n");
#endif
  ...
  case USB_REQ_TYPE_STANDARD:
	...
	//printf("\r\nusbd_cdc_Setup 1 USBD_CtlSendData usbd_cdc_AltSet len:1\r\n");
...
uint8_t  usbd_cdc_DataOut (void *pdev, uint8_t epnum)
{
  ...
  DCD_EP_PrepareRx(pdev,
                   ...
                   CDC_DATA_OUT_PACKET_SIZE);
#if 0
  printf("\r\n 1 len:%d { " , CDC_DATA_OUT_PACKET_SIZE);
  	for (i=0; i<CDC_DATA_OUT_PACKET_SIZE; i++)
  		printf("%02x ", USB_Rx_Buffer[i]);
    printf("} \r\n ");
#endif
