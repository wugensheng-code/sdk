...
USBH_Status USBH_DeInit(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost)
{
  ...
  phost->device_prop.speed = HPRT0_PRTSPD_HIGH_SPEED;
...
void USBH_Process(USB_OTG_CORE_HANDLE *pdev , USBH_HOST *phost)
{
 ...
 // USB_OTG_GINTMSK_TypeDef  intmsk;
  ...
  case HOST_IDLE :
	  //cpu_time0 = fetch_rv_cycle();
	  //printf("----USBH_Process HOST_IDLE----\n\r");
	 // delay_ms(1);
	  delay_us(1000);

	  //cpu_time1= fetch_rv_cycle();
	 // printf("USBH_Process HOST_IDLE CPU CYCLE: %d\n\r",cpu_time1-cpu_time0);
    if (HCD_IsDeviceConnected(pdev) )
    {
    	/*
    	speed = HCD_GetCurrentSpeed(pdev);

        while (speed !=HPRT0_PRTSPD_HIGH_SPEED)
        {
    		// printf("Current (HS) USB!\r\n");
    		 USB_OTG_BSP_uDelay(100);
    		 speed = HCD_GetCurrentSpeed(pdev);
    	}
        printf("55555\n\r");
        USB_SPD_Get(pdev);
        */
     // printf("pdev->host.ConnSts %d\n\r",pdev->host.ConnSts);
     // printf("pdev->host.PortEnabled %d\n\r",pdev->host.PortEnabled);
     ...
     // printf("----USBH_Process HOST_IDLE HCD_ResetPort----\n\r");
    }
  ...
  case HOST_WAIT_PRT_ENABLED:
	  printf("----USBH_Process HOST_WAIT_PRT_ENABLED----\n\r");
	//  USB_SPD_Get(pdev);
  ...
  case HOST_DEV_ATTACHED :
	  printf("----USBH_Process HOST_DEV_ATTACHED----\n\r");
    ...
    //USB_SPD_Get(pdev); //if remove this function, cpu will dead
    USB_OTG_BSP_uDelay(1);
    break;
    
  case HOST_ENUMERATION:     
    /* Check for enumeration status */
	  printf("----USBH_Process HOST_ENUMERATION----\n\r");
  ...
  case HOST_USR_INPUT:
	  printf("----USBH_Process HOST_USR_INPUT----\n\r");
  ...
  case HOST_CLASS_REQUEST:  
	  printf("----USBH_Process HOST_CLASS_REQUEST----\n\r");
    ...
    break;    
  case HOST_CLASS:   
	  printf("----USBH_Process HOST_CLASS----\n\r");
	  //USB_OTG_BSP_mDelay(10); //wait 10m , cpu is dead.
  ...
  case HOST_CTRL_XFER:
    /* process control transfer state machine */
	  printf("----USBH_Process HOST_CTRL_XFER----\n\r");
  ...
  case HOST_ERROR_STATE:
    /* Re-Initialize Host for new Enumeration */
	  //printf("----USBH_Process HOST_ERROR_STATE----\n\r");
    USBH_DeInit(pdev, phost);
    phost->usr_cb->DeInit();
  ...
  case HOST_DEV_DISCONNECTED :
	  printf("----USBH_Process HOST_DEV_DISCONNECTED----\n\r");
    ...
    //USB_OTG_ReadCoreItr(pdev); //added by cai
...
USBH_Status USBH_HandleControl (USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost)
{
 ...
 // printf("phost->Control.state:%d\r\n",phost->Control.state);
  ...
  case CTRL_SETUP_WAIT:
   ...
   // printf("hc num out: %d URB_Status:%d \n\r",phost->Control.hc_num_out,URB_Status);
...
void USB_SPD_Get(USB_OTG_CORE_HANDLE *pdev)
{
	...
	speed = HCD_GetCurrentSpeed(pdev);
	 ...
	 if(speed==HPRT0_PRTSPD_HIGH_SPEED)
	    ...
	    {
	        printf("Current (FS) USB!\r\n");
	    }
	    else if(speed==HPRT0_PRTSPD_LOW_SPEED)
	    {
	        printf("Current (LS) USB!\r\n");
	    }
	    else
	    {
	        printf("Current USB Speed ERR!\r\n");
	    }
}
...
#define GPVNDCTL_DISULPIDRV		31
#define GPVNDCTL_VSTSDONE		27
#define GPVNDCTL_VSTSBSY		26
#define GPVNDCTL_NEWRQ		    25
#define GPVNDCTL_RW		        22

#define GPVNDCTL_REGADDR_MSK		  0X3F
#define GPVNDCTL_REGADDR		      16

#define GPVNDCTL_VCTL_MSK		  0XFF
#define GPVNDCTL_VCTL		      8

#define GPVNDCTL_DATA_MASK		0xff
#define GPVNDCTL_DATA           0




uint8_t phy_read(USB_OTG_CORE_HANDLE *pdev,u8 addr)
{

	uint32_t phy_ctrl_r=0;
	uint32_t rd_phy=0;
	uint8_t rdata=0;
	//printf("GPVNDCTL ADDR: 0X%x\n\r",(uint32_t)(&pdev->regs.GREGS->GPVNDCTL));
	 rd_phy = USB_OTG_READ_REG32(&pdev->regs.GREGS->GPVNDCTL);
	// printf("rd_phy:0x%x\n\r",rd_phy);
	phy_ctrl_r |= (1 << GPVNDCTL_NEWRQ); //SET FOR NEW RQ

	phy_ctrl_r &= ~(1 << GPVNDCTL_RW); //0 FOR READ, 1 FOR WRITE

	phy_ctrl_r |= ( (0x2f & GPVNDCTL_REGADDR_MSK)  << GPVNDCTL_REGADDR); //

	phy_ctrl_r |= ( (addr & GPVNDCTL_VCTL_MSK)  << GPVNDCTL_VCTL); //

	//phy_ctrl_r |= ( (0 & GPVNDCTL_REGADDR_MSK)  << GPVNDCTL_REGADDR); // RegAddr,

	//printf("phy_ctrl_r:0x%x\n\r",phy_ctrl_r);
	 USB_OTG_WRITE_REG32(&pdev->regs.GREGS->GPVNDCTL, phy_ctrl_r);
	// printf("phy_ctrl_r after:0x%x\n\r",phy_ctrl_r);
	 //phy_ctrl_r |= (1 << GPVNDCTL_VSTSDONE);
	// USB_OTG_WRITE_REG32(&pdev->regs.GREGS->GPVNDCTL, phy_ctrl_r);
	// printf("phy_ctrl_r after:0x%x\n\r",phy_ctrl_r);

	 rd_phy = USB_OTG_READ_REG32(&pdev->regs.GREGS->GPVNDCTL);
	// printf("rd_phy:0x%x\n\r",rd_phy);
	while ( ((rd_phy >> GPVNDCTL_VSTSDONE) & 0x1) ==0  )
	{
		 rd_phy = USB_OTG_READ_REG32(&pdev->regs.GREGS->GPVNDCTL);
		// printf("rd_phy:0x%x\n\r",rd_phy);
	}

	rdata = rd_phy & GPVNDCTL_DATA_MASK;
	printf("read phy addr:0x%x = 0x%x\n\r",addr,rdata);

	return rdata;

}

uint8_t phy_write(USB_OTG_CORE_HANDLE *pdev,u8 addr,u8 wdata)
{
	...
	printf("write phy addr:0x%x = 0x%x\n\r",addr,rd_phy & GPVNDCTL_DATA_MASK);

	return rd_phy & GPVNDCTL_DATA_MASK;
...
{
	uint8_t ret=0;
	ret = phy_read(pdev,0xa);
	phy_write(pdev,0xa,0x7);
	USB_OTG_BSP_mDelay(2000);
	phy_write(pdev,0xa,0x27);
	USB_OTG_BSP_mDelay(2000);
...
void my_phy_read(USB_OTG_CORE_HANDLE *pdev)
{
  uint8_t rdata=0;

  rdata = phy_read(pdev,0);
  rdata = phy_read(pdev,1);
  rdata = phy_read(pdev,2);
  rdata = phy_read(pdev,3);
  rdata = phy_read(pdev,0xa);
