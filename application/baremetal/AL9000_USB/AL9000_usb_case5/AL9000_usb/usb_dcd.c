              ...
              USB_OTG_CORE_ID_TypeDef coreID)
{
  ...
  printf("==> DCD_Init  pdev->cfg.dev_endpoints : %d \r\n", pdev->cfg.dev_endpoints);

  synopsysID = USB_OTG_READ_REG32(USB_OTG_FS_BASE_ADDR+0x40);

  printf("Synopsys ID : 0x%x \r\n", synopsysID);
  /* Init ep structure */
...
//#if defined (STM32F446xx) || defined (STM32F469_479xx) || defined (STM32F767xx)	//���Ӷ�STM32F767xx���ж�
  ...
  USB_OTG_CoreInit(pdev);  //set PHY hcx_add ************************
//#else
#if 0 //hcx_add
  printf("===> DCD_Init 8 !!! \n");
    /*Init the Core (common init.) */
                     ...
                     uint32_t   buf_len)
{
  ...
  if ( ep->num == 0 )
  {
	  printf("ep0 setup  USB_OTG_EP0StartXfer\n");
    USB_OTG_EP0StartXfer(pdev , ep);
  }
  else
  {
	 printf("DCD_EP_Tx data: ");
	 for (i=0; i< buf_len; i++)
	  	  printf("%c-",pbuf[i]);
	 printf("\n\r");
    USB_OTG_EPStartXfer(pdev, ep );
