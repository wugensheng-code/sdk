#ifndef __rtthread_DEMO_CONFIG_H
#define __rtthread_DEMO_CONFIG_H
...
#define WDT_TASK_STACK_SIZE         (2048)
#define CAN_SEND_TASK_STACK_SIZE    (2048)
#define CAN_DMARECV_TASK_STACK_SIZE (2048)
#define GPIO_LED_TASK_STACK_SIZE    (2048)
#define DMACAHB_TASK_STACK_SIZE     (2048)
#define IIC_TASK_STACK_SIZE         (2048)
#define UART_TASK_STACK_SIZE        (2048)
#define GBE_TASK_STACK_SIZE         (2048)
#define MMC_TASK_STACK_SIZE         (2048)
#define IIS_TASK_STACK_SIZE         (2048)
#define DMA_TASK_STACK_SIZE         (2048)
#define QSPI_TASK_STACK_SIZE        (2048)
#define DMACAHB2_TASK_STACK_SIZE    (2048)
#define UART_PROC_TASK_STACK_SIZE   (2048)

/* GPIO LED MACRO CONFIG */
#define GPIO_LED_DEVICE_ID          (0)
#define GPIO_LED_PIN_NUM            (14)
#define GPIO_LED_TOGGLE_TIME        (rt_tick_from_millisecond(500))

/* CAN MACRO CONFIG */
#define CAN_DEVICE_ID               (0)
#define CAN_DMACAHB_DEVICE_ID       (0)
#define CAN_SEND_DATA_TIMEOUT_MS    (rt_tick_from_millisecond(500))
#define CAN_RECV_DATA_TIMEOUT_MS    (rt_tick_from_millisecond(3000))
#define CAN_SEND_INTERVAL_TIME      (rt_tick_from_millisecond(100))
#define CAN_DMA_RECV_DATA           (80)

/* WDT MACRO CONFIG */
#define WDT_DEVICE_ID               (0)
#define WDT_FEED_TIME               (rt_tick_from_millisecond(2000))

/* DMACAHB MACRO CONFIG */
#define DMACAHB_DEVICE_ID           (0)
#define DMACAHB_TRANS_TIMEOUT_MS    (rt_tick_from_millisecond(100))
#define DMACAHB_INTERVAL_TIME       (rt_tick_from_millisecond(100))
...
#define IIC_TEST_TIMEOUT_MS         (rt_tick_from_millisecond(1000))
#define IIC_INTERVAL_TIME           (rt_tick_from_millisecond(100))
#define IIC_ADDR_SIZE               (sizeof(AL_U8))
#define IIC_WAIT_WRITE_COMPLETE     (rt_tick_from_millisecond(100))
...
#define UART_INTERVAL_TIME          (rt_tick_from_millisecond(50))
#define UART_QUEUE_BLOCK_TIME       (rt_tick_from_millisecond(100))
#define UART_CMD_LEN_MAX            (20)

/* MMC MACRO CONFIG */
#define MMC_DEVICE_ID               (0)
#define MMC_BLOCK_TIMEOUT_MS        (rt_tick_from_millisecond(5000))
#define MMC_BLOCK_OFFSET            (0)
#define MMC_BLOCK_SIZE              (512)
#define MMC_BLOCK_COUNT             (100)
#define MMC_BLOCK_LEN               (MMC_BLOCK_SIZE * MMC_BLOCK_COUNT)
#define MMC_INTERVAL_TIME           (rt_tick_from_millisecond(200))

/* QSPI MACRO CONFIG */
#define QSPI_DEVICE_ID              (0)
#define QSPI_INTERVAL_TIME          (rt_tick_from_millisecond(200))

/* IIS MACRO CONFIG */
#define IIS_DMA_DEVICE_ID           (0)
#define IIS_DMA_DATA_SIZE           (1024)
#define IIS_DMA_TRANS_TIMEOUT       (rt_tick_from_millisecond(2000))
...
#define IIS_INTERVAL_TIME           (rt_tick_from_millisecond(100))

/* DMA MACRO CONFIG */
#define DMA_DEVICE_ID               (0)
#define DMA_TRANS_DATA_SIZE         (1024)
#define DMA_INTERVAL_TIME           (rt_tick_from_millisecond(100))

/* DMACAHB2 MACRO CONFIG */
#define DMACAHB2_DEVICE_ID          (0)
#define DMACAHB2_TRANS_TIMEOUT_MS   (rt_tick_from_millisecond(100))
#define DMACAHB2_INTERVAL_TIME      (rt_tick_from_millisecond(100))
...
void AlQspi_rtthread_WrEn(AL_QSPI_HalStruct *Handle, AL_U8 *SendData)
{
    AL_S32 Ret = AL_OK;
    ...
    Ret = AlQspi_Hal_SendDataBlock(Handle, SendData, 1, 100000);
    if (Ret != AL_OK) {
        AL_LOG(AL_LOG_LEVEL_ERROR, "AlQspi_rtthread_WrEn error:0x%x\r\n", Ret);
    }
}

void AlQspi_rtthread_SetStatus(AL_QSPI_HalStruct *Handle, AL_U8 *SendData,AL_U8 data)
{
    AL_S32 Ret = AL_OK;
    ...
    if (Ret != AL_OK) {
        AL_LOG(AL_LOG_LEVEL_ERROR, "AlQspi_rtthread_SetStatus error:0x%x\r\n", Ret);
    }
}

void AlQspi_rtthread_WaitWip(AL_QSPI_HalStruct *Handle, AL_U8 *SendData, AL_U8 *RecvData)
{
    AL_S32 Ret = AL_OK;
    ...
    do {
        ...
        if (Ret != AL_OK) {
            AL_LOG(AL_LOG_LEVEL_ERROR, "AlQspi_rtthread_WaitWip error:0x%x\r\n", Ret);
        }
#ifdef QSPI_DEBUG
...
void AlQspi_rtthread_ReadStatus(AL_QSPI_HalStruct *Handle, AL_U8 *SendData, AL_U8 *RecvData)
{
    AL_S32 Ret = AL_OK;
    ...
    if (Ret != AL_OK) {
        AL_LOG(AL_LOG_LEVEL_ERROR, "AlQspi_rtthread_ReadStatus error:0x%x\r\n", Ret);
    }
#ifdef QSPI_DEBUG
...
void AlQspi_rtthread_Erase(AL_QSPI_HalStruct *Handle, AL_U8 *SendData)
{
    AL_S32 Ret = AL_OK;
   ...
   // AlQspi_Dev_DumpReg(Handle->Dev.BaseAddr);
    ...
    if (Ret != AL_OK) {
        AL_LOG(AL_LOG_LEVEL_ERROR, "AlQspi_rtthread_Erase error:0x%x\r\n", Ret);
    }
}

void AlQspi_rtthread_EraseChip(AL_QSPI_HalStruct *Handle, AL_U8 *SendData)
{
    AL_S32 Ret = AL_OK;
    Handle->Dev.Configs.Trans.EnSpiCfg.WaitCycles = 0;
    Handle->Dev.Configs.Trans.TransMode  = QSPI_TX_ONLY;
    ...
    AL_LOG(AL_LOG_LEVEL_DEBUG, "AlQspi_rtthread_EraseChip running\r\n");
    Ret = AlQspi_Hal_SendDataBlock(Handle, SendData, 1, 100000);
    if (Ret != AL_OK) {
        AL_LOG(AL_LOG_LEVEL_ERROR, "AlQspi_rtthread_EraseChip error:0x%x\r\n", Ret);
    }
}


void AlQspi_rtthread_X4_1_1_4_Read(AL_QSPI_HalStruct *Handle, AL_U8 *SendData, AL_U8 Addr, AL_U8 *RecvData)
{
    AL_S32 Ret = AL_OK;
    AL_LOG(AL_LOG_LEVEL_DEBUG, "AlQspi_rtthread_ReadPage\r\n");
    ...
    if (Ret != AL_OK) {
        AL_LOG(AL_LOG_LEVEL_ERROR, "AlQspi_rtthread_ReadPage error:0x%x\r\n", Ret);
    }
    AL_LOG(AL_LOG_LEVEL_DEBUG, "AlQspi_rtthread_ReadPage end\r\n");
}


void AlQspi_rtthread_Write_X4_1_1_4_Page(AL_QSPI_HalStruct *Handle, AL_U8 *SendData, AL_U8 Addr)
{
    AL_S32 Ret = AL_OK;
    ...
    if (Ret != AL_OK) {
        AL_LOG(AL_LOG_LEVEL_ERROR, "AlQspi_rtthread_WritePage error:0x%x\r\n", Ret);
    }
}



void AlQspi_rtthread_X4_1_4_4_ReadPage(AL_QSPI_HalStruct *Handle, AL_U8 *SendData, AL_U8 *RecvData)
{
    AL_S32 Ret = AL_OK;
    AL_LOG(AL_LOG_LEVEL_DEBUG, "AlQspi_rtthread_X4_1_4_4_ReadPage\r\n");
    Handle->Dev.Configs.Trans.TransMode  = QSPI_RX_ONLY;
    Handle->Dev.Configs.Trans.EnSpiCfg.TransType = QSPI_TT0;
    Handle->Dev.Configs.Trans.EnSpiCfg.AddrLength = QSPI_ADDR_L24;
    Handle->Dev.Configs.SpiFrameFormat = SPI_QUAD_FORMAT;
...
void AlQspi_rtthread_ReadPage(AL_QSPI_HalStruct *Handle, AL_U8 *SendData, AL_U8 *RecvData)
{
    AL_S32 Ret = AL_OK;
    Handle->Dev.Configs.Trans.TransMode  = QSPI_EEPROM;
    Handle->Dev.Configs.Trans.EnSpiCfg.AddrLength = QSPI_ADDR_L24;
...
void AlQspi_rtthread_WritePage(AL_QSPI_HalStruct *Handle, AL_U8 *SendData)
{
    AL_S32 Ret = AL_OK;
    Handle->Dev.Configs.SpiFrameFormat  = SPI_STANDARD_FORMAT;
    Handle->Dev.Configs.Trans.EnSpiCfg.WaitCycles = 0;
...
AL_VOID AlQspi_rtthread_ReadId(AL_QSPI_HalStruct *Handle, AL_U8 *SendData, AL_U8 *FlashId)
{
    AL_S32 Ret = AL_OK;
    ...
    if (Ret != AL_OK) {
        AL_LOG(AL_LOG_LEVEL_ERROR, "AlQspi_rtthread_ReadId error:0x%x\r\n", Ret);
    }
    AL_LOG(AL_LOG_LEVEL_DEBUG, "Flash ID:0x%x, 0x%x, 0x%x\r\n", FlashId[0], FlashId[1], FlashId[2]);
}


AL_VOID AlQspi_rtthread_RemoveProtect(AL_QSPI_HalStruct *Handle, AL_U8 *SendData, AL_U8 *RecvData)
{
    AL_S32 Ret = AL_OK;
    AL_U8 Temp;
    Handle->Dev.Configs.SpiFrameFormat  = SPI_STANDARD_FORMAT;
    ...
    Temp = RecvData[0];
    AlQspi_rtthread_WrEn(Handle, SendData);
    ...
    AlQspi_rtthread_WaitWip(Handle, SendData, RecvData);

}

AL_VOID AlQspi_rtthread_MxIcSetQE(AL_QSPI_HalStruct *Handle, AL_U8 *SendData, AL_U8 *RecvData)
{
    AL_S32 Ret = AL_OK;
    AL_U8 Temp;
    Handle->Dev.Configs.Trans.EnSpiCfg.WaitCycles = 0;
    ...
    Ret = AlQspi_Hal_TranferDataBlock(Handle, SendData, 1, RecvData, 1, 100000);
    if (Ret != AL_OK) {
        AL_LOG(AL_LOG_LEVEL_ERROR, "AlQspi_rtthread_MxIcSetQE error:0x%x\r\n", Ret);
    }
    Temp = RecvData[0];

    AL_LOG(AL_LOG_LEVEL_DEBUG, "AlQspi_rtthread_MxIcSetQE Status:%d\r\n", Temp);
    AlQspi_rtthread_WrEn(Handle, SendData);
...
AL_S32 AlQspi_rtthread_SetQuad(AL_QSPI_HalStruct *Handle, AL_U8 *SendData, AL_U8 *RecvData, AL_U8 SetQuadCmd, AL_U8 ReadQuadCmd, AL_U8 QuadPos)
{
    ...
    Ret = AlQspi_Hal_SendDataBlock(Handle, SendData, 2, 100000);
    if (Ret != AL_OK) {
        AL_LOG(AL_LOG_LEVEL_ERROR, "AlQspi_rtthread_SetQuad SetQuadCmd error:0x%x\r\n", Ret);
    }
