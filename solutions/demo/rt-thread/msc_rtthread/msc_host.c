        ...
        while (f_readdir(&DirInfo, &fileinfo) == FR_OK) {
            ...
            if ((fileinfo.fattrib & AM_HID) == 0 && (fileinfo.fattrib & AM_SYS) == 0) {
                if ((fileinfo.fattrib & AM_DIR) == AM_DIR) {
                    USB_LOG_RAW("Folder Name: %s\r\n", fileinfo.fname);
                } else {
                    USB_LOG_RAW("File   Name: %s\r\n", fileinfo.fname);
                }
            }
        }
    } else {
        USB_LOG_RAW("Can't open the directory: %s\n", path);
    }
...
{
    const char *tmp_data = "Anlogic fatfs demo...\r\n";
    ...
    usb_msc_fatfs_scan_file_test("2:/");
...
int usb_msc_fatfs_write_read_test(void)
{
    ...
    USB_LOG_RAW("test fatfs write\r\n");
    res_sd = f_open(&fnew, "2:cherryusb_msc_test.txt", FA_OPEN_ALWAYS | FA_WRITE);
    if (res_sd == FR_OK) {
        res_sd = f_write(&fnew, read_write_buffer, sizeof(read_write_buffer), &fnum);
        if (res_sd == FR_OK) {
            USB_LOG_RAW("write success, write len：%d\n", fnum);
    ...
    res_sd = f_open(&fnew, "2:cherryusb_msc_test.txt", FA_OPEN_EXISTING | FA_READ);
    if (res_sd == FR_OK) {
        res_sd = f_read(&fnew, read_write_buffer, sizeof(read_write_buffer), &fnum);
        if (res_sd == FR_OK) {
            USB_LOG_RAW("read success, read len：%d\n", fnum);
...
void usbh_class_test(void)
{
    USB_LOG_RAW("cherryusb msc host test\r\n");
