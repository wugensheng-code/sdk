...
void printf_monitor(AL_Xmon_HalStruct *Xmon)
{
  	...
  	AL_LOG(AL_LOG_LEVEL_ERROR, "AXIMON arqos_record         = 0x%x\r\n", Xmon->Info.Record.ArQos_Record);
  	AL_LOG(AL_LOG_LEVEL_ERROR, "AXIMON awresptime_max       = 0x%x\r\n", Xmon->Info.AwResptimeMax);
  	AL_LOG(AL_LOG_LEVEL_ERROR, "AXIMON awresptime_acc       = 0x%x\r\n", Xmon->Info.AwResptimeAcc.AwRespTimeAcc);
  	AL_LOG(AL_LOG_LEVEL_ERROR, "AXIMON awresptime_cnt       = 0x%x\r\n", Xmon->Info.AwResptimeCnt);
  	AL_LOG(AL_LOG_LEVEL_ERROR, "AXIMON arresptime_max       = 0x%x\r\n", Xmon->Info.ArResptimeMax);
  	AL_LOG(AL_LOG_LEVEL_ERROR, "AXIMON awresptime_acc       = 0x%x\r\n", Xmon->Info.ArResptimeAcc.ArRespTimeAcc);
  	AL_LOG(AL_LOG_LEVEL_ERROR, "AXIMON arresptime_cnt       = 0x%x\r\n", Xmon->Info.ArResptimeCnt);
  	AL_LOG(AL_LOG_LEVEL_ERROR, "AXIMON bandwidth_wr         = 0x%x\r\n", Xmon->Info.BandWidthWr);
  	AL_LOG(AL_LOG_LEVEL_ERROR, "AXIMON bandwidth_rd         = 0x%x\r\n", Xmon->Info.BandWidthRd);
}

AL_VOID Al_Xmon_tc(AL_VOID)
{
    AL_S32 Ret = AL_OK;
...
// CMOCKA_TC_DEFINE(Al_Xmon_tc, NULL, NULL, NULL);