...
#define AL_FATFS_DEMO_LSEEK_OFFSET      (0x50)
static AL_U8 WriteBuffer[AL_FATFS_DEMO_READ_WRITE_SIZE];
static AL_U8 ReadBuffer[AL_FATFS_DEMO_READ_WRITE_SIZE];
static AL_U8 WorkBuffer[FF_MAX_SS];
#endif
...
AL_S32 AlFatfs_Demo_SdReadWrite(AL_VOID)
{
    ...
    AL_LOG(AL_LOG_LEVEL_DEBUG, "Sd read/write test!\r\n");
    /* Mount immediately */
    Ret = f_mount(&FileSystem, "0:", 1);
    if (Ret == FR_NO_FILESYSTEM) {
        AL_LOG(AL_LOG_LEVEL_DEBUG, "Sd no file system, Wait for sd mkfs...\r\n");
        ...
        AL_LOG(AL_LOG_LEVEL_DEBUG, "Mount sd card success!\r\n");
    }
    ...
    AL_LOG(AL_LOG_LEVEL_DEBUG, "Create directory success!\r\n");
    ...
    AL_LOG(AL_LOG_LEVEL_DEBUG, "Open/create file success!\r\n");
    ...
    AL_LOG(AL_LOG_LEVEL_DEBUG, "Write data in byte %d in file %s success!\r\n", OpsNum, FileName);
    ...
    AL_LOG(AL_LOG_LEVEL_DEBUG, "Close after write file success!\r\n");
    ...
    AL_LOG(AL_LOG_LEVEL_DEBUG, "Open existing file success!\r\n");
    ...
    AL_LOG(AL_LOG_LEVEL_DEBUG, "Lseek size in byte %d in file %s success!\r\n", AL_FATFS_DEMO_LSEEK_OFFSET, FileName);

    Ret = f_read(&File, (const void *)ReadBuffer, AL_FATFS_DEMO_READ_WRITE_SIZE - AL_FATFS_DEMO_LSEEK_OFFSET, &OpsNum);
    if (Ret != AL_OK || OpsNum != AL_FATFS_DEMO_READ_WRITE_SIZE - AL_FATFS_DEMO_LSEEK_OFFSET) {
        AL_LOG(AL_LOG_LEVEL_ERROR, "Read file error!\r\n");
        goto ERROR_READ;
    }
    AL_LOG(AL_LOG_LEVEL_DEBUG, "Read data in byte %d in file %s success!\r\n", OpsNum, FileName);
    ...
    AL_LOG(AL_LOG_LEVEL_DEBUG, "Close after read file success!\r\n");

    Ret = memcmp(WriteBuffer + AL_FATFS_DEMO_LSEEK_OFFSET, ReadBuffer, AL_FATFS_DEMO_READ_WRITE_SIZE - AL_FATFS_DEMO_LSEEK_OFFSET);
    if (Ret != 0) {
...
ERROR_MOUNT:
SUCCESS:
    AL_LOG(AL_LOG_LEVEL_DEBUG, "Ret is 0x%x\r\n", Ret);
    AL_LOG(AL_LOG_LEVEL_DEBUG, "Sd read/write test done!\r\n", Ret);
    return Ret;
}
